...
import { Button, LoadingError, OrderStatus, Pagination } from "../components";
...
import ShipmentTableShimmer from "../components/shimmers/ShipmentTable.shimmer";
...
const Shipment = () => {
  ...
  const [fetchOrders, isLoading, error] = useLoading(async (page) => {
    const { data } = await orderService.getMany({ isShipment: true, page });
    setOrders((prev) => ({
      ...prev,
      [page]: data.orders,
    }));
  ...
  const [handleShipment, isDispatching, dispatchError] = useLoading(
    async (id) => {
      ...
      setOrders((prev) => ({
        ...prev,
        [page]: prev[page].map((o) =>
          o._id === id ? { ...data, cart: o.cart } : o
        ),
      }));
    }
  );
  ...
  const downloadInvoice = async (id) => {
    const order = orders[page].find((order) => order._id === id);
  ...
  return (
    <div className="mx-auto p-2 sm:p-4 min-h-[45vh]">
      <div className="overflow-x-auto lg:rounded-tl-lg lg:rounded-tr-lg">
        {error ? (
          <LoadingError error={error} retry={() => fetchOrders(page)} />
        ) : isLoading ? (
          <ShipmentTableShimmer />
        ) : (
