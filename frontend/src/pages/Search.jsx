...
const Search = () => {
  ...
  const filterProducts = useCallback(() => {
    // TODO: fetch the filtered products from server
  ...
  useEffect(() => {
    if (!searchValue || allProducts.length === 0) {
      navigate("/"); // TODO: navigate and send the state to scroll to the product section
    } else filterProducts();
  }, [searchValue]);

  if (searchProducts.length === 0) {
    return (
      <div className="w-full text-center mt-10 font-bold text-2xl min-h-72">
        No Products Found
      </div>
    );
  }
